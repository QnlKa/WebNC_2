  ...
  try {
    ...
    if (username && password) {
      ...
      if (admin) {
        ...
        res.status(200).json({
          success: false,
          message: "Tên tài khoảng hoặc mật khẩu không đúng",
        });
      }
    } else {
      res.status(200).json({
        success: false,
        message: "Vui lòng nhập tài khoảng và mật khẩu",
      });
    }
  } catch (e) {
...
router.get("/token", JwtUtil.checkToken, function (req, res) {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Token hợp lệ", token: token });
  } catch (e) {
...
router.post("/categories", JwtUtil.checkToken, async function (req, res) {
  try {
    ...
    if (name && image) {
      ...
      const ID = "CAT" + increNumber.toString().padStart(4, "0");
      ...
      res
        .status(200)
        .json({ success: false, message: "Vui lòng nhập đủ trường dữ liệu" });
    }
  } catch (e) {
...
router.put("/categories/:id", JwtUtil.checkToken, async function (req, res) {
  try {
    ...
    const name = req.body.name;
    const image = req.body.image;
    const category = { _id: _id, name: name, image: image };
    if (name && image) {
      ...
      res
        .status(200)
        .json({ success: false, data, message: "Nhập thiếu trường dữ liệu" });
    }
  } catch (e) {
    res.json({ fail: e });
  }
...
router.delete("/categories/:id", JwtUtil.checkToken, async function (req, res) {
  try {
    ...
    if (_id) {
      ...
      res
        .status(200)
        .json({ success: false, message: "vui lòng chọn đối tượng để xoá" });
    }
  } catch (e) {
    res.status(500).json({ success: false, e });
  }
...
router.get("/products", JwtUtil.checkToken, async function (req, res) {
  try {
    ...
    // return
    const data = { products: products, noPages: noPages, curPage: curPage };
...
router.post("/products", JwtUtil.checkToken, async function (req, res) {
  try {
    ...
    if (name && price && cid && image && decription) {
      ...
      res
        .status(200)
        .json({ data: result, success: true, message: "Thêm thành công" });
    } else {
      res
        .status(200)
        .json({ success: false, message: "Vui lòng nhập đúng trường dữ liệu" });
    }
  } catch (e) {
    res.status(500).json({ success: false, e });
  }
...
router.put("/products/:id", JwtUtil.checkToken, async function (req, res) {
  try {
    ...
    if (name && price && cid && image && decription && _id) {
      ...
      res
        .status(200)
        .json({ data: result, success: true, message: "Sửa thành công" });
    } else {
      res.status(200).json({
        success: false,
        message: "Vui lòng không để trống trường dữ liệu",
      });
    }
  } catch (e) {
    res.status(500).json({ success: false, e });
